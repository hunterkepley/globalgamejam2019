...
func generateRooms(doneRooms chan bool, amount int, startPos pixel.Vec, roomSize pixel.Vec) {
	for i := 0; i < amount; i++ {
		rX := startPos.X
		rY := startPos.Y
		rW := roomSize.X
		rH := roomSize.Y
		if i != 0 {
			rX = rooms[i-1].pos.X + rooms[i-1].size.X
			rY = randFloat64(rooms[i-1].pos.Y-rH+500, rooms[i-1].pos.Y+rooms[i-1].size.Y-500)
		}
		rPos := pixel.V(rX, rY)
		rSize := pixel.V(rW, rH)
		room := newRoom(rPos, rSize, 0)
		for j := 0; j < len(rooms); j++ {
			for room.intersectsRoom(rooms[j]) {
				room.pos.X += 50
			}
		}
